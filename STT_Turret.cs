...
public class TurretTargeting
{
    ...
    [Tooltip("GameObject with following tags will be identified as enemies")]
...
public class STT_Turret : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Add only "Player" tag to the tagsToFire array
        targeting.tagsToFire = new string[] { "Player" };
    }
    ...
    private void Shooting()
    {
        ...
        if (Physics.Raycast(VFX.muzzle.position, VFX.muzzle.transform.forward, out hit, parameters.radius))
        {
            if (CheckTags(hit.collider) == true)
            {
                ...
                // Find the PlayerStats component on the collider's GameObject
                PlayerStats playerStats = hit.collider.GetComponent<PlayerStats>();

                // Check if the playerStats is not null and then damage the player
                if (playerStats != null)
                {
                    playerStats.Damage(parameters.power);
                }
            }
    ...
    {
        return toMatch.CompareTag("Player");
    }
